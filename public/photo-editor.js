...
export async function openPhotoEditor(blob){
  ...
  const setMode=value=>{mode=value;document.querySelectorAll('[data-photo-mode]').forEach(button=>button.classList.toggle('selected',button.dataset.photoMode===value));status.textContent=value==='crop'?'Drag over the area you want to keep.':value==='draw'?'Draw directly on the photo with your finger.':'Enter text, tap Place Text, then tap the photo.'};
