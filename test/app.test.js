...
test('is installable and offline capable',async()=>{const manifest=JSON.parse(await readFile(new URL('../public/manifest.webmanifest',import.meta.url),'utf8')),sw=await readFile(new URL('../public/service-worker.js',import.meta.url),'utf8');assert.equal(manifest.display,'standalone');assert.match(sw,/quiet-notes-iphone-v9/)});
